...
{
	border = border || 0;
	bounds = (bounds == null) ? new mxRectangle(this.x, this.y, this.width, this.height) : bounds;
	
	if (this.shape != null && this.shape.stencil != null && this.shape.stencil.aspect == 'fixed')
	{
		const direction = this.style[mxConstants.STYLE_DIRECTION];
		const inverse = (direction == mxConstants.DIRECTION_NORTH || direction == mxConstants.DIRECTION_SOUTH);
		const sw = inverse ? this.shape.stencil.h0 : this.shape.stencil.w0;
		const sh = inverse ? this.shape.stencil.w0 : this.shape.stencil.h0;
		const s = Math.min(bounds.width / sw, bounds.height / sh);
...
mxCellState.prototype.setAbsoluteTerminalPoint = function(point, isSource)
{
	...
	else
	{
		...
		{
			this.absolutePoints = [];
			this.absolutePoints.push(null, point);
		}
...
{
 	const clone = new mxCellState(this.view, this.cell,
	...
	if (this.absolutePoints != null)
	{
		...
		for (let i = 0; i < this.absolutePoints.length; i++)
		{
			clone.absolutePoints[i] = this.absolutePoints[i] == null ?
					null : this.absolutePoints[i].clone();
		}
	}
