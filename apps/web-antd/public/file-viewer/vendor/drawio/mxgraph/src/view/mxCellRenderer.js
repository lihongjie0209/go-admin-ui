...
mxCellRenderer.prototype.createShape = function(state)
{
	...
	if (state.style != null)
	{
		...
		const stencil = (mxCellRenderer.defaultShapes[name] == null) ?
...
{
	return (name == null) ? null : mxCellRenderer.defaultShapes[name];
};
...
mxCellRenderer.prototype.checkPlaceholderStyles = function(state)
{
	...
	if (state.style != null)
	{
		...
		const styles = [mxConstants.STYLE_FILLCOLOR, mxConstants.STYLE_STROKECOLOR,
...
mxCellRenderer.prototype.inheritFontStyle = function(state, field, key)
{
	...
	{
		const graph = state.view.graph;
		const pstate = graph.getView().getState(
...
{
	const shape = (key == mxConstants.STYLE_FONTCOLOR) ?
	...
	if (shape != null)
	{
		...
		if (referenced != null)
		{
			...
			if (rstate != null)
			{
				const rshape = (key == mxConstants.STYLE_FONTCOLOR) ? rstate.text : rstate.shape;
...
mxCellRenderer.prototype.redrawLabel = function(state, forced)
{
	...
	const dialect = (isForceHtml) ? mxConstants.DIALECT_STRICTHTML :
		state.view.graph.dialect;
	const overflow = state.style[mxConstants.STYLE_OVERFLOW] || 'visible';
	const convertToSvg = mxUtils.getValue(state.style, 'convertToSvg', '0') == '1';
...
mxCellRenderer.prototype.isTextShapeInvalid = function(state, shape)
{
	function check(property, stylename, defaultValue)
	{
		...
		{
			result = Number.parseFloat(shape[property]) - Number.parseFloat(shape.spacing) !=
				(state.style[stylename] || defaultValue);
		}
		else
		{
			result = shape[property] != ((state.style[stylename] == null) ?
				defaultValue : state.style[stylename]);
		}
...
{
	text = (text == null) ? state.text : text;
	...
	if (lw != null)
	{
		bounds.width = Number.parseFloat(lw) * scale;
	}
	...
	for (let i = 0; i < shapes.length; i++)
	{
		if (shapes[i] != null && shapes[i].node != null)
		{
			...
			if (temp != null && temp.parentNode != null && temp.nextSibling != shapes[i].node)
			{
				...
				{
					temp.parentNode.append(shapes[i].node);
				}
				else
				{
			...
			else if (temp == null)
			{
				...
				if (shapes[i].node.parentNode == state.view.graph.container)
				{
					...
					{
						shapes[i].node.parentNode.append(shapes[i].node);
					}
				}
