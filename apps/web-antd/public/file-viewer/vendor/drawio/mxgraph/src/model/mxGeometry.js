...
mxGeometry.prototype.translate = function(dx, dy, ignorePosition)
{
	...
	{
		this.x = Number.parseFloat(this.x) + dx;
		this.y = Number.parseFloat(this.y) + dy;
	}

	// Translates the source point
	if (this.sourcePoint != null)
	{
		this.sourcePoint.x = Number.parseFloat(this.sourcePoint.x) + dx;
		this.sourcePoint.y = Number.parseFloat(this.sourcePoint.y) + dy;
	}
	
	// Translates the target point
	if (this.targetPoint != null)
	{
		this.targetPoint.x = Number.parseFloat(this.targetPoint.x) + dx;
		this.targetPoint.y = Number.parseFloat(this.targetPoint.y) + dy;		
	}
	...
	if (this.TRANSLATE_CONTROL_POINTS && this.points != null)
	{
		for (let i = 0; i < this.points.length; i++)
		{
			if (this.points[i] != null)
			{
				this.points[i].x = Number.parseFloat(this.points[i].x) + dx;
				this.points[i].y = Number.parseFloat(this.points[i].y) + dy;
			}
		}
	}
};
...
{
	sx = Number.parseFloat(sx);
	sy = Number.parseFloat(sy);

	// Translates the source point
	if (this.sourcePoint != null)
	{
		this.sourcePoint.x = Number.parseFloat(this.sourcePoint.x) * sx;
		this.sourcePoint.y = Number.parseFloat(this.sourcePoint.y) * sy;
	}
	
	// Translates the target point
	if (this.targetPoint != null)
	{
		this.targetPoint.x = Number.parseFloat(this.targetPoint.x) * sx;
		this.targetPoint.y = Number.parseFloat(this.targetPoint.y) * sy;		
	}
	...
	if (this.points != null)
	{
		for (let i = 0; i < this.points.length; i++)
		{
			if (this.points[i] != null)
			{
				this.points[i].x = Number.parseFloat(this.points[i].x) * sx;
				this.points[i].y = Number.parseFloat(this.points[i].y) * sy;
			}
		}
	}
	...
	if (!this.relative)
	{
		this.x = Number.parseFloat(this.x) * sx;
		this.y = Number.parseFloat(this.y) * sy;
		...
		this.width = Number.parseFloat(this.width) * sx;
		this.height = Number.parseFloat(this.height) * sy;
	}
};
