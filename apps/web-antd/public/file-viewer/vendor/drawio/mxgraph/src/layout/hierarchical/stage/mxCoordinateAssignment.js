...
mxCoordinateAssignment.prototype.minNode = function(model)
{
	...
	while (nodeList.length > 0 && count <= maxTries)
	{
		...
		const medianNextLevel = this.medianXValue(nextLayerConnectedCells,
				rankValue + 1);
		const medianPreviousLevel = this.medianXValue(previousLayerConnectedCells,
				rankValue - 1);

		const numConnectedNeighbours = numNextLayerConnected
...
mxCoordinateAssignment.prototype.rankMedianPosition = function(rankValue, model, nextRankValue)
{
	...
	for (var i = 0; i < weightedValues.length; i++)
	{
		...
		if (nextRankValue < rankValue)
		{
			nextLayerConnectedCells = [...cell.getPreviousLayerConnectedCells(rankValue)];
		}
		else
		{
			nextLayerConnectedCells = [...cell.getNextLayerConnectedCells(rankValue)];
		}
			...
			if (cell.isVertex())
			{
				...
				for (let k = 0; k < 2; k++)
				{
							...
							&& currentCells.length > 0)
					{
						...
						const requiredWidth = (connectedEdgeCount + 1)
								* this.prefHozEdgeSep;
						...
						{
							const numActualEdges = connectedEdges[j].edges
									.length;
		...
		for (let i = 0; i < cell.edges.length; i++)
		{
			...
			for (let j = loopStart; (cell.maxRank != cell.minRank) && j != loopLimit; j += loopDelta)
			{
				...
				if (this.orientation == mxConstants.DIRECTION_NORTH ||
					this.orientation == mxConstants.DIRECTION_SOUTH)
				{
					newPoints.push(new mxPoint(positionX, topChannelY), new mxPoint(positionX, bottomChannelY));
				}
				else
				{
					newPoints.push(new mxPoint(topChannelY, positionX), new mxPoint(bottomChannelY, positionX));
				}
