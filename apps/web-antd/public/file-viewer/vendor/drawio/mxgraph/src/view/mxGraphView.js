...
mxGraphView.prototype.validate = function(cell)
{
	...
	const cell = (cell != null) ? cell : ((this.currentRoot != null) ?
	...
	this.setGraphBounds((graphBounds == null) ?
		this.getEmptyBounds() : graphBounds);
...
mxGraphView.prototype.getBoundingBox = function(state, recurse, update)
{
	...
	if (state != null)
	{
		...
		if (recurse)
		{
			...
			for (let i = 0; i < childCount; i++)
			{
				const bounds = this.getBoundingBox(this.getState(
...
mxGraphView.prototype.updateCellState = function(state)
{
	...
	if (state.cell != this.currentRoot)
	{
		...
		if (geo != null)
		{
			...
			{
				offset = (geo.offset == null) ? this.EMPTY_POINT : geo.offset;
...
mxGraphView.prototype.updateEdgeState = function(state, geo)
{
	...
	const hasGtp = typeof geo.getTerminalPoint === 'function';
...
mxGraphView.prototype.updateBoundsFromStencil = function(state)
{
	...
	if (state != null && state.shape != null && state.shape.stencil != null && state.shape.stencil.aspect == 'fixed')
	{
		...
		const direction = state.style[mxConstants.STYLE_DIRECTION];
		const inverse = (direction == mxConstants.DIRECTION_NORTH || direction == mxConstants.DIRECTION_SOUTH);
		const sw = inverse ? state.shape.stencil.h0 : state.shape.stencil.w0;
		const sh = inverse ? state.shape.stencil.w0 : state.shape.stencil.h0;
		const s = Math.min(state.width / sw, state.height / sh);
...
mxGraphView.prototype.updatePoints = function(edge, points, source, target)
{
		...
		edge.absolutePoints.length > 0)
	{
		...
		if (edgeStyle != null)
		{
			...
			const trgBounds = (src != trg) ? this.updateBoundsFromStencil(trg) : null;
...
mxGraphView.prototype.getFloatingTerminalPoint = function(edge, start, end, source)
{
	...
	border += Number.parseFloat(edge.style[(source) ?
...
mxGraphView.prototype.getPerimeterBounds = function(terminal, border)
{
	...
	if (terminal != null)
	{
		border += Number.parseFloat(terminal.style[mxConstants.STYLE_PERIMETER_SPACING] || 0);
	}
...
mxGraphView.prototype.createHtml = function()
{
	...
	if (container != null)
	{
		...
		this.canvas.append(this.backgroundPane);
		this.canvas.append(this.drawPane);
		this.canvas.append(this.overlayPane);
		this.canvas.append(this.decoratorPane);
