...
mxStencil.prototype.parseDescription = function()
{
	...
	this.fgNode = this.desc.querySelectorAll('foreground')[0];
	this.bgNode = this.desc.querySelectorAll('background')[0];
	...
	this.aspect = (aspect == null) ? 'variable' : aspect;
...
{
	graph = (graph == null) ? ((shape != null &&
		shape.state != null) ? shape.state.view.graph : null) : graph;
...
mxStencil.prototype.getStyleWithNone = function(shape, key, graph)
{
	...
	return this.cachedUnresolvedStyle == null ?
		null : this.cachedUnresolvedStyle[key];
};
...
{
	const defaultValue = node.getAttribute('default');
	let result = mxUtils.getValue(shape.style, value);

	if (result == null && graph != null)
	{
		const temp = this.getStyleWithNone(shape, value, graph);
