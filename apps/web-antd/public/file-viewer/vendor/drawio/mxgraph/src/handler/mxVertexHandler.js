...
mxVertexHandler.prototype.createSizer = function(cursor, index, size, fillColor)
{
	...
	else
	{
		sizer.dialect = (this.graph.dialect == mxConstants.DIALECT_SVG) ?
				mxConstants.DIALECT_SVG : mxConstants.DIALECT_MIXEDHTML;
	...
	if (this.selectionBorder != null)
	{
		...
		else
		{
			...
			if (index == mxEvent.ROTATION_HANDLE)
			{
				...
				this.startAngle = (dx == 0) ? 0 : Math.atan(dy / dx) * 180 / Math.PI + 90;
...
mxVertexHandler.prototype.rotateVertex = function(me)
{
	...
	this.currentAlpha = (dx == 0) ? ((dy < 0) ? 180 : 0) : Math.atan(dy / dx) * 180 / Math.PI + 90;
...
mxVertexHandler.prototype.resizeVertex = function(me)
{
	...
	this.bounds = new mxRectangle(((this.parentState == null) ? tr.x * scale : this.parentState.x) +
		(this.unscaledBounds.x) * scale, ((this.parentState == null) ? tr.y * scale : this.parentState.y) +
...
mxVertexHandler.prototype.updateLivePreview = function(me)
{
	...
	const originalWidth = this.state.cell.geometry.width;
	const originalHeight = this.state.cell.geometry.height;
...
mxVertexHandler.prototype.moveToFront = function()
{
		...
		this.state.shape.node.nextSibling != this.state.text.node)))
	{
		...
		{
			this.state.shape.node.parentNode.append(this.state.shape.node);
		}
		
		if (this.state.text != null && this.state.text.node != null)
		{
			this.state.text.node.parentNode.append(this.state.text.node);
		}
	}
};
...
{
	gridEnabled = (gridEnabled == null) ? this.graph.gridEnabled : gridEnabled && this.graph.gridEnabled;
...
mxVertexHandler.prototype.redrawHandles = function()
{
	...
	if (this.rotationShape != null)
	{
		var alpha = mxUtils.toRadians((this.currentAlpha == null) ? this.state.style[mxConstants.STYLE_ROTATION] || '0' : this.currentAlpha);
		var cos = Math.cos(alpha);
		var sin = Math.sin(alpha);
...
mxVertexHandler.prototype.createSizers = function()
{
		...
		this.state.width >= 2 && this.state.height >= 2))
	{
		...
		if (resizable)
		{
			if (!this.singleSizer)
			{
				sizers.push(this.createSizer('nw-resize', i++), this.createSizer('n-resize', i++));
				sizers.push(this.createSizer('ne-resize', i++), this.createSizer('w-resize', i++));
				sizers.push(this.createSizer('e-resize', i++), this.createSizer('sw-resize', i++));
