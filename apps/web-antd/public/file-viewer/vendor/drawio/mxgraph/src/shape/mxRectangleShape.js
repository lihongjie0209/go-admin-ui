...
{
	const footer = this.getFooterSize() * this.scale;
...
{
	const footer = Math.min(h, this.getFooterSize());
...
{
	const color = mxUtils.getValue(this.style,
		mxConstants.STYLE_FOOTER_COLOR, this.stroke);
	const r = (this.isRounded) ? this.getArcSize(w, h) : 0;
	const rr = Math.min(w / 2, r);
