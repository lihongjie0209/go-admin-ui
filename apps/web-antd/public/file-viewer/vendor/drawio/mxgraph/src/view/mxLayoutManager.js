...
mxLayoutManager.prototype.cellsMoved = function(cells, evt)
{
	if (cells != null && evt != null)
	{
		...
		{
			const layout = this.getLayout(model.getParent(cells[i]), mxEvent.MOVE_CELLS);
...
mxLayoutManager.prototype.cellsResized = function(cells, bounds, prev)
{
	if (cells != null && bounds != null && prev != null)
	{
		...
		for (let i = 0; i < cells.length; i++)
		{
			const layout = this.getLayout(model.getParent(cells[i]), mxEvent.RESIZE_CELLS);
...
{
	const layout = this.getLayout(cell, (bubble) ?
