 ...
 */
mxGraphHierarchyModel.prototype.SOURCESCANSTARTRANK = 100_000_000;
	...
	for (let i = 0; i < vertices.length; i++)
	{
		...
		for (let j = 0; j < conns.length; j++)
		{
					...
					!layout.isVertexIgnored(cell))
			{
				...
				// process them again
				const undirectedEdges = layout.getEdgesBetween(vertices[i],
						cell, false);
				const directedEdges = layout.getEdgesBetween(vertices[i],
						cell, true);
...
mxGraphHierarchyModel.prototype.initialRank = function()
{
	...
	while (startNodes.length > 0)
	{
		...
		for (var i = 0; i < layerDeterminingEdges.length; i++)
		{
			...
			if (internalEdge.temp[0] == 5_270_620)
			{
