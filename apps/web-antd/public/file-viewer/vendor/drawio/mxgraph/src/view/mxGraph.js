...
function mxGraph(container, model, renderHint, stylesheet, rendering)
{
	...
	this.model = (model == null) ? new mxGraphModel() : model;
	this.multiplicities = [];
	this.imageBundles = [];
	this.cellRenderer = this.createCellRenderer();
	this.setSelectionModel(this.createSelectionModel());
	this.setStylesheet((stylesheet == null) ? this.createStylesheet() : stylesheet);
	this.view = this.createGraphView();
	this.view.rendering = (rendering == null) ? this.view.rendering : rendering;
...
{
	cell = (cell == null) ? this.model.getRoot() : cell;
	this.removeCellOverlays(cell);
...
mxGraph.prototype.setCellWarning = function(cell, warning, img, isSelect)
{
	...
	{
		img = (img == null) ? this.warningImage : img;
		
		// Creates the overlay with the image and warning
		const overlay = new mxCellOverlay(img,
	...
	if (this.isEnabled() && !mxEvent.isConsumed(evt) && !mxe.isConsumed())
	{
		if (cell != null)
		{
			if (this.isTransparentClickEvent(evt))
			{
				...
				const tmp = this.getCellAt(me.graphX, me.graphY, null, null, null,
...
mxGraph.prototype.getBorderSizes = function()
{
	...
	return new mxRectangle(mxUtils.parseCssNumber(css.paddingLeft) +
			((css.borderLeftStyle == 'none') ? 0 : mxUtils.parseCssNumber(css.borderLeftWidth)),
		mxUtils.parseCssNumber(css.paddingTop) +
			((css.borderTopStyle == 'none') ? 0 : mxUtils.parseCssNumber(css.borderTopWidth)),
		mxUtils.parseCssNumber(css.paddingRight) +
			((css.borderRightStyle == 'none') ? 0 : mxUtils.parseCssNumber(css.borderRightWidth)),
		mxUtils.parseCssNumber(css.paddingBottom) +
			((css.borderBottomStyle == 'none') ? 0 : mxUtils.parseCssNumber(css.borderBottomWidth)));
};
...
mxGraph.prototype.fit = function(border, keepOrigin, margin, enabled, ignoreWidth, ignoreHeight, maxHeight)
{
	...
	{
		border = (border == null) ? this.getBorder() : border;
		keepOrigin = (keepOrigin == null) ? false : keepOrigin;
		margin = (margin == null) ? 0 : margin;
		enabled = (enabled == null) ? true : enabled;
		ignoreWidth = (ignoreWidth == null) ? false : ignoreWidth;
		ignoreHeight = (ignoreHeight == null) ? false : ignoreHeight;
...
mxGraph.prototype.getCurrentCellStyle = function(cell, ignoreState)
{
	...
	return (state == null) ? this.getCellStyle(cell) : state.style;
};
...
{
	resolve = (resolve == null) ? true : resolve;
...
mxGraph.prototype.postProcessCellStyle = function(cell, style)
{
	if (style != null)
	{
		...
		if (image != null && typeof image.substring === 'function' &&
			image.slice(0, 11) == 'data:image/')
		{
			if (image.slice(0, 20) == 'data:image/svg+xml,<')
			{
				// Required for FF and IE11
				image = image.slice(0, 19) + encodeURIComponent(image.slice(19));
			}
			else if (image.slice(0, 22) != 'data:image/svg+xml,%3C')
			{
...
mxGraph.prototype.updateGroupBounds = function(cells, border, moveGroup, topBorder, rightBorder, bottomBorder, leftBorder)
{
	...
	moveGroup = (moveGroup == null) ? false : moveGroup;
	topBorder = (topBorder == null) ? 0 : topBorder;
	rightBorder = (rightBorder == null) ? 0 : rightBorder;
	bottomBorder = (bottomBorder == null) ? 0 : bottomBorder;
	leftBorder = (leftBorder == null) ? 0 : leftBorder;
...
{
	allowInvalidEdges = (allowInvalidEdges == null) ? true : allowInvalidEdges;
	let clones = null;
		...
		x, y, width, height, style, relative)
{
	...
	geometry.relative = (relative == null) ? false : relative;
...
mxGraph.prototype.addCells = function(cells, parent, index, source, target, absolute)
{
	...
	try
	{
		this.cellsAdded(cells, parent, index, source, target, (absolute == null) ? false : absolute, true);
...
{
	includeEdges = (includeEdges == null) ? true : includeEdges;
...
mxGraph.prototype.cellsRemoved = function(cells)
{
	if (cells != null && cells.length > 0)
	{
		...
		try
		{
			...
			let contractParents = null;

			if (this.isContractParentsOnRemove())
			{
				contractParents = [];
				const seen = new mxDictionary();
...
mxGraph.prototype.splitEdge = function(edge, cells, newEdge, dx, dy, x, y, parent)
{
	...
	parent = (parent == null) ? this.model.getParent(edge) : parent;
...
{
	recurse = (recurse == null) ? false : recurse;
...
mxGraph.prototype.updateAlternateBounds = function(cell, geo, willCollapse)
{
	if (cell != null && geo != null)
	{
		...
		if (geo.alternateBounds == null)
		{
			...
			{
				const gridEnabled = mxUtils.getValue(style, mxConstants.STYLE_AUTOSIZE_GRID,
...
{
	ignoreChildren = (ignoreChildren == null) ? false : ignoreChildren;
	...
	if (cell != null)
	{
		...
		try
		{
			...
			if (geo != null)
			{
				...
				if (fixedWidth)
				{
					w = geo.width - 2 * Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING, 2)) -
						Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_LEFT, 0)) -
						Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_RIGHT, 0));
				}
...
{
	gridEnabled = (gridEnabled == null) ? this.gridEnabled : gridEnabled;
	let result = null;
		...
		if (!this.model.isEdge(cell))
		{
			...
			if (this.getImage(state) != null || style[mxConstants.STYLE_IMAGE] != null)
			{
				if (style[mxConstants.STYLE_SHAPE] == mxConstants.SHAPE_LABEL)
				{
					...
					{
						dx += Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_IMAGE_WIDTH,
							mxLabel.prototype.imageSize));
					}
					
					if (style[mxConstants.STYLE_ALIGN] != mxConstants.ALIGN_CENTER)
					{
						dy += Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_IMAGE_HEIGHT,
							mxLabel.prototype.imageSize));
					}
				}
			}
			...
			// Adds spacings
			dx += 2 * Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING, 2));
			dx += Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_LEFT, 2));
			dx += Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_RIGHT, 2));

			dy += 2 * Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING, 2));
			dy += Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_TOP, 2));
			dy += Number.parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_BOTTOM, 2));
			...
			if (value != null && value.length > 0)
			{
				...
				const size = mxUtils.getSizeForString(value, fontSize,
...
{
	recurse = (recurse == null) ? this.isRecursiveResize() : recurse;
	...
	if (cell != null)
	{
		...
		if (parent != null && p != null && !this.isCellCollapsed(parent))
		{
			...
			if (geo != null && !geo.relative)
			{
				...
				const padding = parseFloat(mxUtils.getValue(style,
...
mxGraph.prototype.translateCell = function(cell, dx, dy)
{
	...
	{
		dx = Number.parseFloat(dx);
		dy = Number.parseFloat(dy);
		geo = geo.clone();
		geo.translate(dx, dy, this.model.isEdge(cell));

		if (!geo.relative && this.model.isVertex(cell) && !this.isAllowNegativeCoordinates())
		{
			geo.x = Math.max(0, Number.parseFloat(geo.x));
			geo.y = Math.max(0, Number.parseFloat(geo.y));
		}
		...
		if (geo.relative && !this.model.isEdge(cell))
		{
			...
			{
				geo.offset.x = Math.round(Number.parseFloat(geo.offset.x + dx));
				geo.offset.y = Math.round(Number.parseFloat(geo.offset.y + dy));
			}
		}
...
{
	sizeFirst = (sizeFirst == null) ? true : sizeFirst;
...
mxGraph.prototype.getConnectionConstraint = function(edge, terminal, source)
{
	...
	if (x != null)
	{
		...
		if (y != null)
		{
			point = new mxPoint(Number.parseFloat(x), Number.parseFloat(y));
		}
	}
	...
	if (point != null)
	{
		...
		dx = Number.parseFloat(edge.style[(source) ? mxConstants.STYLE_EXIT_DX : mxConstants.STYLE_ENTRY_DX]);
		dy = Number.parseFloat(edge.style[(source) ? mxConstants.STYLE_EXIT_DY : mxConstants.STYLE_ENTRY_DY]);
...
mxGraph.prototype.getConnectionPoint = function(vertex, constraint, round)
{
	...
	if (vertex != null && constraint.point != null)
	{
		...
		const r2 = vertex.style[mxConstants.STYLE_ROTATION] || 0;
...
{
	includeEdges = (includeEdges == null) ? false : includeEdges;
	let result = null;
				...
				if (geo != null)
				{
					...
					if (this.model.isEdge(cells[i]))
					{
						...
						if (bbox != null && this.model.isVertex(parent) && mxUtils.indexOf(
							(ancestors == null) ? cells : ancestors, parent) >= 0)
						{
							var tmp = this.getBoundingBoxFromGeometry([parent], false,
								(ancestors == null) ? cells : ancestors,
								includeStrokeWidth);
						...
						if (geo.relative)
						{
							if (this.model.isVertex(parent) && parent != this.view.currentRoot)
							{
								...
								if (tmp != null)
								{
									...
									if (mxUtils.indexOf((ancestors == null) ? cells : ancestors, parent) >= 0)
									{
										bbox.x += tmp.x;
										bbox.y += tmp.y;
									}
		...
		if (this.dialect == mxConstants.DIALECT_SVG)
		{
			...
			if (dx == 0 && dy == 0)
			{
				...
				if (this.shiftPreview1 != null)
				{
					...
					this.container.append(canvas.parentNode);
			...
			else
			{
				...
				if (this.shiftPreview1 == null)
				{
					...
					if (this.shiftPreview1.firstChild != null)
					{
						...
						{
							this.container.append(this.shiftPreview1);
						}
					}
					
					if (this.shiftPreview2.firstChild != null)
					{
						this.container.append(this.shiftPreview2);
					}
				}
...
mxGraph.prototype.center = function(horizontal, vertical, cx, cy)
{
	...
	vertical = (vertical == null) ? true : vertical;
	cx = (cx == null) ? 0.5 : cx;
	cy = (cy == null) ? 0.5 : cy;
...
{
	center = (center == null) ? this.centerZoom : center;
	...
	{
		const rect = new mxRectangle(state.x * factor, state.y * factor,
...
mxGraph.prototype.scrollCellToVisible = function(cell, center)
{
	...
	if (state != null)
	{
		const bounds = new mxRectangle(x + state.x, y + state.y, state.width,
			state.height);
...
{
	cell = (cell == null) ? this.model.getRoot() : cell;
	context = (context == null) ? new Object() : context;
...
mxGraph.prototype.isWrapping = function(cell)
{
	const style = this.getCurrentCellStyle(cell);
	const state = this.view.getState(cell);
	const dir = style[mxConstants.STYLE_TEXT_DIRECTION];
	...
	const usesSvg = mxUtils.getValue(style, 'convertToSvg', '0') == '1' &&
...
mxGraph.prototype.getTooltip = function(state, node, x, y)
{
	...
	if (state != null)
	{
			...
			node.parentNode == state.control.node))
		{
			...
			tip = mxUtils.htmlEntities(mxResources.get(tip) || tip).replaceAll('\\n', '<br>');
		}
		...
		if (tip == null && state.overlays != null)
		{
			state.overlays.visit(function(id, shape)
			{
				...
				{
					tip = mxUtils.htmlEntities(shape.overlay.toString()).replaceAll('\\n', '<br>');
				}
			});
		}
...
mxGraph.prototype.getStartSize = function(swimlane, ignoreState)
{
	...
	const size = parseInt(mxUtils.getValue(style,
...
mxGraph.prototype.getActualStartSize = function(swimlane, ignoreState)
{
	...
	{
		const style = this.getCurrentCellStyle(swimlane, ignoreState);
		const size = parseInt(mxUtils.getValue(style, mxConstants.STYLE_STARTSIZE,
...
mxGraph.prototype.getDropTarget = function(cells, evt, cell, clone)
{
	...
	const pt = mxUtils.convertPoint(this.container,
...
{
	vertices = (vertices == null) ? true : vertices;
	edges = (edges == null) ? true : edges;
...
{
	parent = (parent == null) ? this.getDefaultParent() : parent;
	vertices = (vertices == null) ? false : vertices;
	edges = (edges == null) ? false : edges;
...
{
	incoming = (incoming == null) ? true : incoming;
	outgoing = (outgoing == null) ? true : outgoing;
	includeLoops = (includeLoops == null) ? true : includeLoops;
	recurse = (recurse == null) ? false : recurse;
...
{
	sources = (sources == null) ? true : sources;
	targets = (targets == null) ? true : targets;
 ...
 */
 ...
 {
	const p = mxUtils.convertPoint(this.container,
		mxEvent.getClientX(evt), mxEvent.getClientY(evt));
		...
		if (parent != null)
		{
			...
			for (let i = 0; i < childCount; i++)
			{
					...
					(ignoreFn == null || !ignoreFn(state)))
				{
					...
					const hit = (intersection != null && model.isVertex(cell) && mxUtils.intersects(intersection, box)) ||
...
{
	isolate = (isolate == null) ? false : isolate;
	invert = (invert == null) ? false : invert;
	const roots = [];
...
mxGraph.prototype.traverse = function(vertex, directed, func, edge, visited, inverse)
{
	...
	{
		directed = (directed == null) ? true : directed;
		inverse = (inverse == null) ? false : inverse;
...
{
	return [...this.getSelectionModel().cells];
};
...
mxGraph.prototype.fireMouseEvent = function(evtName, me, sender)
{
		...
		mxClient.IS_TOUCH && (mxEvent.isTouchEvent(me.getEvent()) || mxEvent.isPenEvent(me.getEvent()))))
	{
		...
		else if ((this.isMouseDown || evtName == mxEvent.MOUSE_UP) && this.fireDoubleClick)
		{
			...
			const valid = (cell != null) || ((mxEvent.isTouchEvent(me.getEvent()) || mxEvent.isPenEvent(me.getEvent())) &&
