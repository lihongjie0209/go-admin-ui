...
mxCellEditor.prototype.setAlign = function (align)
{
	...
	{
		const dir = mxUtils.getValue(state.style, mxConstants.STYLE_TEXT_DIRECTION,
...
mxCellEditor.prototype.resize = function()
{
	...
	{
		const m = mxUtils.getAlignmentAsPoint((this.align != null) ? this.align :
		...
		if (!this.autoSize || (state.style[mxConstants.STYLE_OVERFLOW] == 'fill'))
		{
		...
		else
	 	{
			...
			const svgWrap = mxUtils.getValue(state.style, 'convertToSvg', '0') == '1' &&
...
{
	let size = mxUtils.getValue(state.style, mxConstants.STYLE_FONTSIZE,
		mxConstants.DEFAULT_FONTSIZE);
	let family = mxUtils.getValue(state.style, mxConstants.STYLE_FONTFAMILY,
		mxConstants.DEFAULT_FONTFAMILY);
	const bold = (mxUtils.getValue(state.style, mxConstants.STYLE_FONTSTYLE, 0) &
			mxConstants.FONT_BOLD) == mxConstants.FONT_BOLD;
	const italic = (mxUtils.getValue(state.style, mxConstants.STYLE_FONTSTYLE, 0) &
			mxConstants.FONT_ITALIC) == mxConstants.FONT_ITALIC;
	const txtDecor = [];

	const parentStyle = this.graph.getCellStyle(this.graph.model.getParent(state.cell), true);

	if (size == 'inherit')
	{
		size = (parentStyle == null) ? mxConstants.DEFAULT_FONTSIZE : mxUtils.getValue(parentStyle, mxConstants.STYLE_FONTSIZE,
			mxConstants.DEFAULT_FONTSIZE);
	}
	
	if (family == 'inherit')
	{
		family = (parentStyle == null) ? mxConstants.DEFAULT_FONTFAMILY : mxUtils.getValue(parentStyle, mxConstants.STYLE_FONTFAMILY,
			mxConstants.DEFAULT_FONTFAMILY);
	}
	...
	let color = mxUtils.getValue(state.style, mxConstants.STYLE_FONTCOLOR);
	...
	const backgroundColor = this.getBackgroundColor(state);
	...
	let dir = mxUtils.getValue(state.style, mxConstants.STYLE_TEXT_DIRECTION,
		mxConstants.DEFAULT_TEXT_DIRECTION);
	let align = mxUtils.getValue(state.style, mxConstants.STYLE_ALIGN,
	...
	{
		const valign = mxUtils.getValue(state.style, mxConstants.STYLE_VERTICAL_ALIGN,
