...
{
	const shape = new mxRectangleShape(mxRectangle.fromRectangle(bounds),
...
mxEdgeHandler.prototype.createMarker = function()
{
	...
	{
		const model = self.graph.getModel();
		const other = self.graph.view.getTerminalPort(state,
...
mxEdgeHandler.prototype.getHandleForEvent = function(me)
{
	...
	if (this.state != null)
	{
		...
		const hit = (!this.allowHandleBoundsCheck) ? null :
...
mxEdgeHandler.prototype.moveLabel = function(edgeState, x, y)
{
	...
	if (geometry != null)
	{
		...
		if (geometry.relative)
		{
			...
			const pt = this.graph.getView().getRelativePoint(edgeState, x, y);
			geometry.x = Math.round(pt.x * 10_000) / 10_000;
...
{
	const pt = mxUtils.convertPoint(this.graph.container, mxEvent.getClientX(evt),
...
mxEdgeHandler.prototype.drawPreview = function()
{
	try
	{
		if (this.isLabel)
		{
			...
			const bounds = new mxRectangle(Math.round(this.label.x - b.width / 2),
				Math.round(this.label.y - b.height / 2), b.width, b.height);
...
mxEdgeHandler.prototype.refresh = function()
{
	if (this.state != null)
	{
		...
		const geo = this.graph.getCellGeometry(this.state.cell);
		this.edgeStyle = (geo == null) ? null : this.graph.view.getEdgeStyle(
			this.state, geo.points, source, target);
		this.abspoints = this.getSelectionPoints(this.state);
		this.points = [];
		...
		if (this.isHandlesVisible())
		{
			...
			{
				this.labelShape.node.parentNode.append(this.labelShape.node);
			}
		}
	}
};
