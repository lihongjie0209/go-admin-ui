...
function mxSvgCanvas2D(root, styleEnabled)
{
	 ...
	 */
	this.styleEnabled = (styleEnabled == null) ? false : styleEnabled;
	...
	if (svg != null)
	{
		...
		if (tmp.length > 0)
		{
			this.defs = svg.querySelectorAll('defs')[0];
		}
		...
		{
			this.defs.append(this.createStyle());
		}
	}
};
...
(function()
{
	...
	mxSvgCanvas2D.prototype.useAbsoluteIds = !mxClient.IS_CHROMEAPP &&
		!mxClient.IS_EDGE && document.querySelectorAll('base').length > 0;
})();
...
{
	return Number.parseFloat(parseFloat(value).toFixed(2));
};
...
{
	return (str == null) ? null : this.foAltText;
};
...
mxSvgCanvas2D.prototype.createAlternateContent = function(fo, x, y, w, h, str, align, valign, wrap, format, overflow, clip, rotation)
{
	...
	{
		const dy = (valign == mxConstants.ALIGN_TOP) ? 1 :
...
mxSvgCanvas2D.prototype.createGradientId = function(start, end, alpha1, alpha2, direction)
{
	...
	const prefix = (this.idPrefix != '') ? this.idPrefix : 'mx';
	
	return (`${prefix  }-gradient-${ 
		start  }-${  end  }-${  dir}`).
		replaceAll(/^[^a-z]+|[^\w:.-]+/gi, '_');
};
...
mxSvgCanvas2D.prototype.getSvgGradient = function(start, end, alpha1, alpha2, direction)
{
	...
	const id = this.createGradientId(start.cssText, end.cssText, alpha1, alpha2, direction);
...
{
	const parts = ['mx-pattern', type,
		Math.round(strokeSize * 100),
		color.light, color.dark,
		Math.round(scale * 100)];

	// Removes illegal characters from gradient ID
	return parts.join('-').toLowerCase().replaceAll(/^[^a-z]+|[^\w:.-]+/gi, '_');
};
...
mxSvgCanvas2D.prototype.getFillPattern = function(type, strokeSize, color, scale)
{
	...
	const id = this.createFillPatternId(type, strokeSize, color, scale);
	...
	const vt = this.viewTranslate;
	const tx = (vt != null) ? this.format(vt.x * scale) : 0;
	const ty = (vt != null) ? this.format(vt.y * scale) : 0;
	const hasRotate = (type !== 'dots');
...
{
	const baseSW = strokeSize * 1.5;
	const size = this.format(10 + baseSW);
...
mxSvgCanvas2D.prototype.createZigZagLinePattern = function(strokeSize, color, scale)
{
	...
	const s1_4 = size / 4, s3_4 = 3 * size / 4;
	...
	fillPattern.append(path);
	return fillPattern;
};

mxSvgCanvas2D.prototype.createCrossHatchPattern = function(strokeSize, color, scale)
{
	const baseSW = strokeSize * 0.5;
	const size = this.format(1.5 * (10 + baseSW));
...
{
	const size = this.format(10 + strokeSize);
...
{
	this.root.append(this.createTolerance(node));
};
...
mxSvgCanvas2D.prototype.updateFill = function()
{
	...
	let fill, lightFill, isGradient = false;
	...
	if (s.fillColor != null)
	{
		if (s.gradientColor != null && s.gradientColor != mxConstants.NONE)
		{
			...
			if (this.root.ownerDocument == document && this.useAbsoluteIds)
			{
				...
				var base = this.getBaseUrl().replaceAll(/([(\)])/g, String.raw`\$1`);
				fill = `url(${  base  }#${  id  })`;
			}
			else
			{
		...
		{
			const cssFill = this.getLightDarkColor(String(s.fillColor).toLowerCase());
			lightFill = cssFill.light;
			fill = cssFill.cssText;
		}
	}

	const baseStrokeWidth = Math.max(this.minStrokeWidth, Math.max(0.01,
		this.format(s.strokeWidth)));
	const pId = (s.fillStyle == null || s.fillStyle == 'auto' || s.fillStyle == 'solid') ? null :
...
mxSvgCanvas2D.prototype.updateStroke = function()
{
	...
	const cssStroke = this.getLightDarkColor(String(s.strokeColor).toLowerCase());
...
mxSvgCanvas2D.prototype.image = function(x, y, w, h, src, aspect, flipH, flipV, clipPath)
{
	...
	aspect = (aspect == null) ? true : aspect;
	flipH = (flipH == null) ? false : flipH;
	flipV = (flipV == null) ? false : flipV;
...
mxSvgCanvas2D.prototype.convertHtml = function(val)
{
	...
	else
	{
		...
		ta.innerHTML = val.replaceAll('&amp;', '&amp;amp;').
			replaceAll('&#60;', '&amp;lt;').replaceAll('&#62;', '&amp;gt;').
			replaceAll('&lt;', '&amp;lt;').replaceAll('&gt;', '&amp;gt;').
			replaceAll('<', '&lt;').replaceAll('>', '&gt;');
		val = ta.value.replaceAll('&', '&amp;').replaceAll('&amp;lt;', '&lt;').
			replaceAll('&amp;gt;', '&gt;').replaceAll('&amp;amp;', '&amp;').
...
mxSvgCanvas2D.prototype.updateText = function(x, y, w, h, align, valign, wrap, overflow, clip, rotation, dir, node)
{
	if (node != null && node.firstChild != null)
	{
		...
		else if (node.nodeName == 'g')
		{
			...
			const innerG = node.firstChild;
			const blockTextHeight = innerG.getAttribute('data-blockTextHeight');
...
mxSvgCanvas2D.prototype.updateTextNodes = function(x, y, w, h, align, valign, wrap, overflow, clip, rotation, dir, g)
{
	...
	const cssBg = (this.state.fontBackgroundColor != null) ?
		this.getLightDarkColor(this.state.fontBackgroundColor) : null;
	const cssBorder = (this.state.fontBorderColor != null) ?
	...
	mxSvgCanvas2D.createCss(w + this.foreignObjectPadding, h, align, valign, wrap, overflow, clip, dir,
		(cssBg == null) ? null : cssBg.cssText, (cssBorder == null) ? null : cssBorder.cssText,
...
mxSvgCanvas2D.prototype.wrapSvgTextElement = function(textEl, maxWidth)
{
	...
	const fontStyleBits = s.fontStyle;
	const self = this;
	...
	function collectLeaves(parent, inherited)
	{
		for (let i = 0; i < parent.childNodes.length; i++)
		{
			...
			else if (child.nodeType == mxConstants.NODETYPE_ELEMENT)
			{
				...
				const attrNames = ['font-weight', 'font-style', 'fill', 'text-decoration'];
				...
				if (child.style != null)
				{
					if (child.style.cssText != '')
					{
						...
						const props = child.style.cssText.split(';');
				...
				if (dyAttr != null)
				{
					runningDy += Number.parseFloat(dyAttr);
				}
	...
	{
		const parts = segments[i].text.split(/(\s+)/);
	...
	function getFont(segIdx)
	{
		...
		let fw = seg.attrs['font-weight'] || '';
		let fs = seg.attrs['font-style'] || '';
		let ff = fontFamily;
		let fz = fontSize;
		...
		if (seg.css['font-size'])
		{
			...
			{
				fz = Number.parseFloat(sz) * fontSize;
			}
			else if (sz.indexOf('px') >= 0)
			{
				fz = Number.parseFloat(sz);
			}
		}
	...
	for (var i = 0; i < tokens.length; i++)
	{
		...
		const atWordBoundary = i > 0 && tokens[i - 1].isSpace;
	...
	for (var i = 0; i < lines.length; i++)
	{
		...
		const newText = self.createElement('text');
		...
		while (j < line.length)
		{
			...
			if (combined.length > 0)
			{
				...
				const tspan = self.createElement('tspan');
		...
		let lineDescender = lineFontSize * (mxConstants.LINE_HEIGHT - 1);

		// Checks for sup/sub segments on this line and expands line box
		let minDy = 0;
		let maxDy = 0;

		let minDyFontSize = null;
		let maxDyFontSize = null;
...
mxSvgCanvas2D.prototype.wrapSvgBlockElements = function(group, maxWidth, offset)
{
	...
	const lh = Math.round(fontSize * mxConstants.LINE_HEIGHT);
	const children = [];
	...
	for (var i = 0; i < children.length; i++)
	{
		...
		const blockFontSize = parseFloat(textEl.getAttribute('font-size')) || fontSize;
		const wrapped = this.wrapSvgTextElement(textEl, maxWidth);

		if (wrapped != null)
		{
			var origY = Number.parseFloat(textEl.getAttribute('y')) || 0;
			origY += heightDelta;

			// Uses y positions from wrapSvgTextElement which account for
			// per-line font size variations (mixed font sizes within text)
			const firstWrappedY = parseFloat(wrapped.elements[0].getAttribute('y')) || 0;
			const lineFontSize = parseFloat(textEl.getAttribute('data-line-font-size')) || blockFontSize;
			...
			const ascentAdjustment = lineFontSize - firstWrappedY;
			origY -= ascentAdjustment;

			for (let j = 0; j < wrapped.elements.length; j++)
			{
				const newEl = wrapped.elements[j];
				const wrappedY = parseFloat(newEl.getAttribute('y')) || 0;
	...
	if (this.textEnabled && str != null)
	{
		...
		if (this.foEnabled && format == 'html')
		{
			...
			if (div != null)
			{
				...
				{
					if (dir != null && dir.slice(0, 9) != 'vertical-')
					{
...
mxSvgCanvas2D.prototype.createClip = function(x, y, w, h)
{
	...
	while (document.querySelector(`#${tmp}`) != null)
	{
		tmp = `${id  }-${  ++counter}`;
	}
...
mxSvgCanvas2D.prototype.setClip = function(node, c)
{
	...
	{
		const id = node.getAttribute('clip-path').replace(/url\(.*#/, '').replace(/\)$/, '');
		const clipNode = document.getElementById(id);
...
mxSvgCanvas2D.prototype.plainText = function(x, y, w, h, str, align, valign, wrap, overflow, clip, rotation, dir, textElement, node, blockTextHeight, htmlContentWidth)
{
	...
	node = (node == null) ? this.addTitle(this.createElement('g')) : node;
	...
	const textHeight = (blockTextHeight != null) ? blockTextHeight :
		size + (lines.length - 1) * lh;

	// For block mode, y offsets on child elements already include baseline
	let cy = (blockTextHeight != null) ? y : y + size - 1;
...
mxSvgCanvas2D.prototype.measureHtmlContentWidth = function(contentElt)
{
	try
	{
		...
		for (let i = 0; i < contentElt.childNodes.length; i++)
		{
			container.append(contentElt.childNodes[i].cloneNode(true));
		}
...
mxSvgCanvas2D.prototype.adjustBlockTextOverflow = function(textGroup, x, cy, w, scale, contentWidth, align)
{
	try
	{
		...
		if (contentWidth > w)
		{
			const shift = (align == mxConstants.ALIGN_CENTER) ?
				(contentWidth - w) / 2 : (contentWidth - w);
			const tx = this.format((x + shift) * scale) + this.textOffset;
			const ty = this.format(cy * scale) + this.textOffset;
...
{
	const s = this.state;
	const cssFontColor = this.getLightDarkColor(s.fontColor);
...
mxSvgCanvas2D.prototype.addTextBackground = function(node, str, x, y, w, h, align, valign, overflow)
{
		...
		s.fontBorderColor != null))
	{
		...
		{
			const n = this.createElement('rect');
			const cssBg = (s.fontBackgroundColor != null &&
				s.fontBackgroundColor != mxConstants.NONE) ?
				this.getLightDarkColor(s.fontBackgroundColor) : null;
			const cssBorder = (s.fontBorderColor != null &&
				s.fontBorderColor != mxConstants.NONE) ?
				this.getLightDarkColor(s.fontBorderColor) : null;

			n.setAttribute('fill', (cssBg == null) ? 'none' : cssBg.light);
			n.setAttribute('stroke', (cssBorder == null) ? 'none' : cssBorder.light);
