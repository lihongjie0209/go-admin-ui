	...
	wrap, clipped, overflow, labelPadding, textDirection)
{
	...
	this.align = (align == null) ? mxConstants.ALIGN_CENTER : align;
	this.valign = (valign == null) ? mxConstants.ALIGN_MIDDLE : valign;
	this.family = (family == null) ? mxConstants.DEFAULT_FONTFAMILY : family;
	this.size = (size == null) ? mxConstants.DEFAULT_FONTSIZE : size;
	this.fontStyle = (fontStyle == null) ? mxConstants.DEFAULT_FONTSTYLE : fontStyle;
	this.spacing = Number.parseInt(spacing || 2);
	this.spacingTop = this.spacing + Number.parseInt(spacingTop || 0);
	this.spacingRight = this.spacing + Number.parseInt(spacingRight || 0);
	this.spacingBottom = this.spacing + Number.parseInt(spacingBottom || 0);
	this.spacingLeft = this.spacing + Number.parseInt(spacingLeft || 0);
	this.horizontal = (horizontal == null) ? true : horizontal;
	this.background = background;
	this.border = border;
	this.wrap = (wrap == null) ? false : wrap;
	this.clipped = (clipped == null) ? false : clipped;
	this.overflow = (overflow == null) ? 'visible' : overflow;
	this.labelPadding = (labelPadding == null) ? 0 : labelPadding;
...
mxText.prototype.paint = function(c, update)
{
	...
	this.updateSvgFilters((c == null) ? s : c.state.scale);
...
mxText.prototype.apply = function(state)
{
	...
	if (this.style != null)
	{
		...
		this.spacing = Number.parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING, this.spacing));
		this.spacingTop = Number.parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_TOP, this.spacingTop - old)) + this.spacing;
		this.spacingRight = Number.parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_RIGHT, this.spacingRight - old)) + this.spacing;
		this.spacingBottom = Number.parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_BOTTOM, this.spacingBottom - old)) + this.spacing;
		this.spacingLeft = Number.parseInt(mxUtils.getValue(this.style, mxConstants.STYLE_SPACING_LEFT, this.spacingLeft - old)) + this.spacing;
...
{
	return (this.style == null) ? false : mxUtils.getValue(this.style,
...
mxText.prototype.getHtmlValue = function()
{
	...
	// Handles trailing newlines to make sure they are visible in rendering output
	val = mxUtils.replaceTrailingNewlines(val, '<div><br></div>');
	val = (this.replaceLinefeeds) ? val.replaceAll('\n', '<br/>') : val;
...
mxText.prototype.redrawHtmlShapeWithCss3 = function()
{
	...
	mxSvgCanvas2D.createCss(w + 2, h, this.align, this.valign, this.wrap, this.overflow, this.clipped, dir,
		(this.background == null) ? null : mxUtils.htmlEntities(this.background),
		(this.border == null) ? null : mxUtils.htmlEntities(this.border),
