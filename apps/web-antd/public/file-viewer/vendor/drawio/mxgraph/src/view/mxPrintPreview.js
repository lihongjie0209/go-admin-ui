...
{
	this.graph = graph;
	this.scale = (scale == null) ? 1 / graph.pageScale : scale;
	this.border = (border == null) ? 0 : border;
	this.pageFormat = mxRectangle.fromRectangle((pageFormat == null) ? graph.pageFormat : pageFormat);
	this.title = (title == null) ? 'Printer-friendly version' : title;
	this.x0 = (x0 == null) ? 0 : x0;
	this.y0 = (y0 == null) ? 0 : y0;
	this.borderColor = borderColor;
	this.pageSelector = (pageSelector == null) ? true : pageSelector;
};
...
mxPrintPreview.prototype.getPageClassCss = function(pageClass, pageFormat)
{
	...
	const size = ((pageFormat.width / ppi)).toFixed(2) + 'in ' +
...
mxPrintPreview.prototype.open = function(css, targetWindow, forcePageBreaks, keepOpen, id, pageFormat, cells)
{
	...
	try
	{
		...
		pageFormat = mxRectangle.fromRectangle((pageFormat == null) ? this.pageFormat : pageFormat);
		...
		this.wnd = (targetWindow == null) ? this.wnd : targetWindow;
		...
		const bounds = mxRectangle.fromRectangle((cells != null) ?
		...
		{
			const dy = i * availableHeight / this.scale - this.y0 / this.scale +
			...
			for (let j = 0; j < hpages; j++)
			{
				...
				const dx = j * availableWidth / this.scale - this.x0 / this.scale +
...
mxPrintPreview.prototype.addPendingCss = function(doc)
{
	if (this.pendingCss != null)
	{
		...
		style.append(doc.createTextNode(this.pendingCss));
...
mxPrintPreview.prototype.writeHead = function(doc, css)
{
	...
	{
		const size = ((pf.width / this.pixelsPerInch)).toFixed(2) + 'in ' +
