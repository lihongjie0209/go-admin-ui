	...
	if (this.path != null)
	{
		...
		if (arguments.length > 2)
		{
			...
			for (let i = 2; i < arguments.length; i += 2)
			{
				...
				this.path.push(this.format((this.lastX + s.dx) * s.scale), this.format((this.lastY + s.dy) * s.scale));
			}
		}
	}
};
...
mxAbstractCanvas2D.prototype.setGradient = function(color1, color2, x, y, w, h, direction, alpha1, alpha2)
{
	const s = this.state;
	s.fillColor = color1;
	s.gradientFillAlpha = (alpha1 == null) ? 1 : alpha1;
	s.gradientColor = color2;
	s.gradientAlpha = (alpha2 == null) ? 1 : alpha2;
...
{
	this.state.fontSize = Number.parseFloat(value);
};
