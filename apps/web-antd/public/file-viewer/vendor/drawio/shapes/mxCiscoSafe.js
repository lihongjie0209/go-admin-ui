...
mxShapeCiscoSafeCompositeIcon.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const bgColor = mxUtils.getValue(this.state.style, 'bgColor', '#C2E0AE');
	const bgDotColor = mxUtils.getValue(this.state.style, 'bgDotColor', '#ffffff');
	const fillColor = mxUtils.getValue(this.state.style, 'fillColor', 'none');
	const shadow = mxUtils.getValue(this.state.style, 'shadow', false);
	const opacity = parseFloat(mxUtils.getValue(this.state.style, 'opacity', false));
	
	c.translate(x, y);
	
	const bgIcon = mxUtils.getValue(this.state.style, 'bgIcon', '');
