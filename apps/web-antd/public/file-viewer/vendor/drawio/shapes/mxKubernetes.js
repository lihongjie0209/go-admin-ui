...
mxShapeKubernetesIcon.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const strokeColor = mxUtils.getValue(this.state.style, 'strokeColor', '#ffffff');

	c.translate(x, y);
	
	const frame = mxStencilRegistry.getStencil('mxgraph.kubernetes.frame');
	...
	const prStencil = mxStencilRegistry.getStencil('mxgraph.kubernetes.' + prIcon);
...
mxShapeKubernetesIcon2.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const fillColor = mxUtils.getValue(this.state.style, 'fillColor', '#ffffff');
	const strokeColor = mxUtils.getValue(this.state.style, 'strokeColor', '#ffffff');
	const hasLabel = mxUtils.getValue(this.state.style, 'kubernetesLabel', 0);
	...
	const prStencil = mxStencilRegistry.getStencil('mxgraph.kubernetes2.' + prIcon);
	c.setFillColor(strokeColor);
