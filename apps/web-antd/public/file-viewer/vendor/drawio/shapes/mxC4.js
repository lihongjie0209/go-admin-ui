...
{
	c.translate(x, y);
	const headSize = Math.min(w / 2, h / 3);
	const r = headSize / 2;
...
{
	const headSize = Math.min(rect.width / 2, rect.height / 3);
...
{
	c.translate(x, y);
	const headSize = Math.min(w * 0.45, h * 0.45);
	const r = headSize / 2;
...
{
	const headSize = Math.min(rect.width * 0.45, rect.height * 0.45);
...
mxShapeC4WebBrowserContainer2.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const strokeColor2 = mxUtils.getValue(this.state.style, 'strokeColor2', '#0E7DAD');
