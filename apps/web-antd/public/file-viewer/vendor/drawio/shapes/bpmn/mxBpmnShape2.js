...
mxBpmnShape.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const bg = mxUtils.getValue(this.style, mxBpmnShape.prototype.miscEnum.BACKGROUND, mxBpmnShape.prototype.eventTypeEnum.NONE);
...
mxBpmnShape.prototype.redrawPath = function(c, x, y, w, h, layer)
{
	...
	else if (layer == mxBpmnShape.prototype.miscEnum.SYMBOL)
	{
		...
		const s = mxUtils.getValue(this.style, mxBpmnShape.prototype.miscEnum.SYMBOL, null);
...
mxBpmnShape.prototype.symbols = {
		...
		'message': function(c, x, y, w, h, layer, isInverse)
		{
			...
			const fc = mxUtils.getValue(this.style, "fillColor", "none");
...
mxShapeBpmnGateway.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const gwType = mxUtils.getValue(this.style, 'gwType', 'event');
	const outline = mxUtils.getValue(this.style, 'outline', 'none');
	const symbol = mxUtils.getValue(this.style, 'symbol', 'standard');
	let tmpW = w * 0.5;
	const tmpH = h * 0.5;
...
{
	const bpmnShapeType = mxUtils.getValue(this.style, 'bpmnShapeType', 'task');
	const taskMarker = mxUtils.getValue(this.style, 'taskMarker', 'abstract');
	const strokeWidth = mxUtils.getValue(this.style, 'strokeWidth', 1);
	const dashed = mxUtils.getValue(this.style, 'dashed', false);
	const inset = mxUtils.getValue(this.style, 'indent', 3);
	let offsetY = 14;
	const rectOutline = mxUtils.getValue(this.style, 'rectOutline', 'single');
	
	c.translate(x, y);

	const superShape = mxCellRenderer.prototype.getShape('mxgraph.basic.rect');
	...
	const isLoopSub = mxUtils.getValue(this.style, 'isLoopSub', false);
	const isLoopStandard = mxUtils.getValue(this.style, 'isLoopStandard', false);
	const isLoopMultiParallel = mxUtils.getValue(this.style, 'isLoopMultiParallel', false);
	const isLoopMultiSeq = mxUtils.getValue(this.style, 'isLoopMultiSeq', false);
	const isLoopComp = mxUtils.getValue(this.style, 'isLoopComp', false);
	const isAdHoc = mxUtils.getValue(this.style, 'isAdHoc', false);
...
mxShapeBpmn2Task2.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const dashed = mxUtils.getValue(this.style, 'dashed', false);
	const inset = mxUtils.getValue(this.style, 'indent', 3);
	let offsetY = 14 + strokeWidth * 0.5;
	...
	const previousSetStrokeWidth = c.setStrokeWidth;
	c.setStrokeWidth = function() {};
...
{
	const superShape = mxCellRenderer.prototype.getShape('note');
	superShape.prototype.paintVertexShape.call(this, c, x, y, w, h);

	const trType = mxUtils.getValue(this.style, 'bpmnTransferType', 'none');
	const isColl = mxUtils.getValue(this.style, 'isCollection', false);
	c.setShadow(false);
...
{
	const bpmnConvType = mxUtils.getValue(this.style, 'bpmnConversationType', 'conv');
	const strokeWidth = mxUtils.getValue(this.style, 'strokeWidth', 1);
