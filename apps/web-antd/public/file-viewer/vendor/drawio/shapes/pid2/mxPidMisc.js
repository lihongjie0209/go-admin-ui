...
mxShapePidFan.prototype.foreground = function(c, x, y, w, h)
{
	...
	const type = mxUtils.getValue(this.style, mxShapePidFan.prototype.cst.FAN_TYPE, 'common');
...
{
	const type = mxUtils.getValue(this.style, mxShapePidColumn.prototype.cst.COLUMN_TYPE, 'common');
...
{
	const wheelSize = Math.min(h, w * 0.5);
	...
	const dist = w - wheelSize * 1.8;
	const startX = wheelSize * 0.9;
	const step = wheelSize * 0.7;

	for (let i = 0; i < dist; i = i + step)
	{
