...
{
	const isShadow = mxUtils.getValue(this.style, 'network2IconShadow', false);
	const bgColor = mxUtils.getColorValue(this.style, 'network2bgFillColor', 'none');
	c.translate(x, y);
...
{
	const gradientColor = mxUtils.getColorValue(this.style, 'gradientColor', 'none');
	const gradientDir = mxUtils.getValue(this.style, 'gradientDirection', 'south');
	const fillColor = mxUtils.getColorValue(this.style, 'fillColor', '#ffffff');
	const fillColor = mxUtils.getColorValue(this.style, 'fillColor', '#ffffff');
	const bgColor = mxUtils.getColorValue(this.style, 'network2bgFillColor', 'none');
...
{
	const stencilName = mxUtils.getValue(this.style, 'network2Icon', null);
	let stencil = null;
	...
	if (bgColor != 'none')
	{
		const stencilW = mxUtils.getValue(this.style, 'network2IconW', 1);
		const stencilH = mxUtils.getValue(this.style, 'network2IconH', 1);
		const xOff = mxUtils.getValue(this.style, 'network2IconXOffset', 0);
		const yOff = mxUtils.getValue(this.style, 'network2IconYOffset', 0);
...
{
	const stencilName = mxUtils.getValue(this.style, 'network2Icon', null) + "_shadow";
	let stencil = null;
