...
{
	const bodyColor = mxUtils.getValue(this.style, 'bodyColor', '#000000');
...
mxShapeMockupiPhone.prototype.foreground = function(c, x, y, w, h, rSize)
{
	...
	const fillColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '');
	const bgStyle = mxUtils.getValue(this.style, mxMockupC.BG_STYLE, mxMockupC.BG_FLAT_GREEN);
...
{
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '');
	const strokeColor2 = mxUtils.getValue(this.style, mxMockupC.STYLE_STROKECOLOR2, '');
...
{
	const fillColor2 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR2, '');
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '');
	const strokeColor2 = mxUtils.getValue(this.style, mxMockupC.STYLE_STROKECOLOR2, '');
...
{
	const textStrings = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, '+Button 1, Button 2, Button 3').toString()).split(',');
	const fontColor = mxUtils.getValue(this.style, mxMockupC.STYLE_TEXTCOLOR, '#666666');
	const selectedFontColor = mxUtils.getValue(this.style, mxMockupC.STYLE_TEXTCOLOR2, '#ffffff');
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '17').toString();
	const frameColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '#666666');
	const separatorColor = mxUtils.getValue(this.style, mxMockupC.STYLE_STROKECOLOR2, '#c4c4c4');
	const bgColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '#ffffff');
	const buttonNum = textStrings.length;
	let maxButtonWidth = 0;
	let selectedButton = -1;
	const rSize = 2.5; //rounding size
	...
	for (var i = 0; i < buttonNum; i++)
	{
		...
		const currHeight = (i * minButtonHeight + minButtonHeight * 0.5) * trueH / minH;
...
mxShapeMockupiButtonBar.prototype.foreground = function(c, w, h, rSize, buttonNum, labelOffset, minH, frameColor, separatorColor, bgColor, selectedButton, minButtonHeight)
{
	...
	for (let i = 1; i < buttonNum; i++)
	{
		...
		{
			const currHeight = i * minButtonHeight * h / minH;
...
mxShapeMockupiAppBar.prototype.foreground = function(c, x, y, w, h)
{
	...
	const statusIconColor = mxUtils.getValue(this.style, 'statusIconColor', '#999999');
...
{
	const statusIconColor = mxUtils.getValue(this.style, 'statusIconColor', '#cccccc');
	c.setFillColor(statusIconColor);
	c.setStrokeColor(statusIconColor);
...
{
	const mainText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, 'Main Text'));
	const fontColor = mxUtils.getValue(this.style, mxMockupC.STYLE_TEXTCOLOR2, '#666666').toString();
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '8.5').toString();
	c.translate(x, y);
	this.background(c, x, y, w, h);
	c.setShadow(false);
...
mxShapeMockupiButton.prototype.background = function(c, x, y, w, h)
{
	...
	const fillColor2 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR2, '').toString();
...
{
	const fillColor3 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR3, '').toString();
...
{
	const mainText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, 'Main Text'));
	const fontColor = mxUtils.getValue(this.style, mxMockupC.STYLE_TEXTCOLOR2, '#000000').toString();
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '8').toString();
	c.translate(x, y);
	this.background(c, x, y, w, h);
	c.setShadow(false);
	this.mainText(c, x, y, w, h, mainText, fontSize, fontColor);
...
mxShapeMockupiRadioGroup.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const fontColor = mxUtils.getValue(this.style, mxMockupC.STYLE_TEXTCOLOR2, '#666666').toString();
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '8').toString();
	const optionText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, 'Option 1').toString()).split(',');
...
mxShapeMockupiOnOffButton.prototype.paintVertexShape = function(c, x, y, w, h)
{
	c.translate(x, y);
	w = Math.max(w, 2 * h);
	const state = mxUtils.getValue(this.style, mxMockupC.BUTTON_STATE, mxMockupC.STATE_ON);
...
{
	const handleGradient = mxUtils.getValue(this.style, 'handleGradient', '#ffffff');
	const handleColor = mxUtils.getValue(this.style, 'handleColor', '#888888');
...
{
	const mainText = mxUtils.getValue(this.style, 'mainText', null);
...
{
	const mainText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, 'Main Text').toString()).split(',');
...
{
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '').toString();
...
mxShapeMockupiKeybLetters.prototype.foreground = function(c, x, y, w, h, strokeColor)
{
	...
	c.setShadow(false);
	c.setLineJoin('round');
	const iconColor = mxUtils.getValue(this.style, 'iconColor', '#ffffff');
...
mxShapeMockupiKeybSymbols.prototype.foreground = function(c, x, y, w, h, strokeColor)
{
	...
	c.roundrect(w * 0.7126, h * 0.28, w * 0.0776, h * 0.19, rSizeX, rSizeY);
	c.fill();
	c.text(w * 0.7514, h * 0.375, 0, 0, String.fromCharCode(Number.parseInt('00A3', 16)), mxConstants.ALIGN_CENTER, mxConstants.ALIGN_MIDDLE, 0, null, 0, 0, 0);

	c.roundrect(w * 0.8132, h * 0.28, w * 0.0776, h * 0.19, rSizeX, rSizeY);
	c.fill();
	c.text(w * 0.852, h * 0.375, 0, 0, String.fromCharCode(Number.parseInt('00A5', 16)), mxConstants.ALIGN_CENTER, mxConstants.ALIGN_MIDDLE, 0, null, 0, 0, 0);

	c.roundrect(w * 0.9138, h * 0.28, w * 0.0776, h * 0.19, rSizeX, rSizeY);
	c.fill();
	c.text(w * 0.9526, h * 0.375, 0, 0, String.fromCharCode(Number.parseInt('0095', 16)), mxConstants.ALIGN_CENTER, mxConstants.ALIGN_MIDDLE, 0, null, 0, 0, 0);
...
{
	const directionColor = mxUtils.getValue(this.style, 'directionColor', '#ffffff');
	c.setAlpha(1);
...
{
	let barPos = mxUtils.getValue(this.style, mxMockupC.BAR_POS, '80');
	barPos = Math.min(barPos, 100);
	barPos = Math.max(barPos, 0);
	const pointerPos = mxUtils.getValue(this.style, mxMockupC.POINTER_POS, mxMockupC.POINTER_BOTTOM);
	const rSize = 2.5;
	const deadzone = rSize + 7.5; // rounding + pointer width / 2 
	const virRange = w - 2 * deadzone;
	const truePos = deadzone + virRange * barPos / 100;
	c.setStrokeWidth(0.5);
...
{
	const pointerPos = mxUtils.getValue(this.style, mxMockupC.POINTER_POS, mxMockupC.POINTER_BOTTOM);
	const locText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, 'Some Location'));
	c.setAlpha(1);
	c.setFontColor('#ffffff');
	c.setFontSize(9.5);
...
mxShapeMockupiAlphaList.prototype.paintVertexShape = function(c, x, y, w, h)
{
	c.translate(x, y);
	c.setShadow(false);
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '8');
	const iconColor = mxUtils.getValue(this.style, 'iconColor', '#999999');
...
mxShapeMockupiHorButtonBar.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const separatorColor = mxUtils.getValue(this.style, mxMockupC.STYLE_STROKECOLOR2, '#c4c4c4');
	const bgColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '#ffffff');
	const selectedFillColor = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR2, '#008cff');
	const buttonNum = textStrings.length;
	...
	const trueH = Math.max(h, fontSize * 1.5, 10);
...
{
	const fillColor2 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR2, '#000000');
	const fillColor3 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR3, '#000000');
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '#000000');
	c.setShadow(false);
	c.translate(x, y);
...
mxShapeMockupiURLBar.prototype.foreground = function(c, w, h)
{
	...
	const fieldText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, '').toString()).split(',');
...
{
	const barGradient = mxUtils.getValue(this.style, 'barGradient', '#444444');
	const barColor = mxUtils.getValue(this.style, 'barColor', '#ffffff');
...
{
	const fieldText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, ''));
	c.setFontColor('#ffffff');
	c.setFontStyle(mxConstants.FONT_BOLD);
	c.setFontSize(8);
...
mxShapeMockupiScreenNameBar.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const rSize = 5;
	c.setStrokeWidth(0.5);
	const fillColor2 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR2, '#00ff00');
	c.setFillColor(fillColor2);
	c.rect(0, 0, w, h);
	c.fill();
	c.setShadow(false);
...
{
	const fillColor3 = mxUtils.getValue(this.style, mxMockupC.STYLE_FILLCOLOR3, '#00ff00');
	c.setGradient(fillColor3, fillColor3, 0, 0, w, h * 0.5, mxConstants.DIRECTION_SOUTH, 0.8, 0.1);
	c.rect(0, 0, w, h * 0.5);
	c.fill();

	const fieldText = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.BUTTON_TEXT, ''));
	const textColor = mxUtils.getValue(this.style, mxMockupC.STYLE_TEXTCOLOR, '#00ff00');
...
{
	c.translate(x, y);
	const gridSize = decodeURIComponent(mxUtils.getValue(this.style, mxMockupC.GRID_SIZE, '3,3').toString()).split(',');
...
{
	const fillColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '#00ff00');
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '#00ff00');
	c.setStrokeColor(strokeColor);
	c.setFillColor(fillColor);

	const boxSizeX = w / (parseInt(gridSize[0],10) + (gridSize[0]-1) * 0.5);
	const boxSizeY = h / (parseInt(gridSize[1],10) + (gridSize[1]-1) * 0.5);
...
mxShapeMockupiHomePageControl.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const fillColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '#000000');
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '#000000');
	c.setStrokeColor(strokeColor);
	c.setFillColor(fillColor);

	const rSize = Math.min(h * 0.5, w * 0.05);
...
mxShapeMockupRRect.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const rSize = parseInt(mxUtils.getValue(this.style, mxShapeMockupRRect.prototype.cst.R_SIZE, '10'));
...
mxShapeIosTopButton.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const rSize = parseInt(mxUtils.getValue(this.style, mxShapeIosTopButton.prototype.cst.R_SIZE, '10'));
...
mxShapeIosBottomButton.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const rSize = parseInt(mxUtils.getValue(this.style, mxShapeIosBottomButton.prototype.cst.R_SIZE, '10'));
...
mxShapeMockupFancyRRect.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const rSize = parseInt(mxUtils.getValue(this.style, mxShapeMockupFancyRRect.prototype.cst.R_SIZE, '10'));
