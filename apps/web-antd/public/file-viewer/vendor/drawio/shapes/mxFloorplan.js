...
{
	const wallTh = parseFloat(mxUtils.getValue(this.style, mxFloorplanWall.prototype.cst.WALL_THICKNESS, '10'));
...
{
	const wallTh = parseFloat(mxUtils.getValue(this.style, mxFloorplanWallCorner.prototype.cst.WALL_THICKNESS, '10'));
...
{
	const wallTh = parseFloat(mxUtils.getValue(this.style, mxFloorplanWallU.prototype.cst.WALL_THICKNESS, '10'));
...
{
	const wallTh = parseFloat(mxUtils.getValue(this.style, mxFloorplanRoom.prototype.cst.WALL_THICKNESS, '10'));
...
{
	const wallTh = parseFloat(mxUtils.getValue(this.style, mxFloorplanWindow.prototype.cst.WALL_THICKNESS, '10'));
	c.rect(0, h * 0.5 - wallTh * 0.5, w, wallTh);
	c.fillAndStroke();
...
mxFloorplanStairsRest.prototype.background = function(c, x, y, w, h)
{
	...
	for (let i = 25; i < w - h * 0.5; i = i + step)
	{
		c.moveTo(i, 0);
		c.lineTo(i, h);
	}
...
{
	const dx = w * Math.max(0, Math.min(w, parseFloat(mxUtils.getValue(this.style, 'dx', this.dx))));
	const strokeWidth = parseFloat(mxUtils.getValue(this.style, 'strokeWidth', this.dx));
...
{
	const windowPanes = Math.min(mxUtils.getValue(this.style, 'windowPanes', this.windowPanes), 20);
	
	const d = 2;
	const paneW = (w - 14 - (windowPanes - 1) * d) / windowPanes;
	c.translate(x, y);
...
{
	const dx = w * Math.max(0, Math.min(w, parseFloat(mxUtils.getValue(this.style, 'dx', this.dx))));
	const dx = Math.max(5, dx);
	const dx = Math.min(w - 5, dx);
