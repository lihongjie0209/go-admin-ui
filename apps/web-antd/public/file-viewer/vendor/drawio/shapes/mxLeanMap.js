...
{
	c.translate(x, y);
	const fontSize = parseFloat(mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '8'));
...
{
	const lineH = Math.min(fontSize * 1.5, h);
	c.begin();
	c.moveTo(0, lineH);
	c.lineTo(w, lineH);
	c.stroke();
...
mxLeanSharedProcess.prototype.foreground = function(c, w, h, fontSize)
{
	...
	const offset = 0;
	c.begin();
	const strokeWidth = parseFloat(mxUtils.getValue(this.style, mxConstants.STYLE_STROKEWIDTH, '2'));
	...
	while (start < h + w)
	{
		...
		if (start > lineH)
		{
			const startX = Math.max(0, start - h); 
			const startY = Math.min(start, h); 

			const endX = Math.min(start - lineH, w);

			const endY = lineH;
			const endY = Math.max(start - w, lineH);
...
mxLeanProductionKanban.prototype.getConstraints = function(style, w, h)
{
	...
	constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 4, h), new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, 25));
...
mxLeanSignalKanban.prototype.getConstraints = function(style, w, h)
{
	...
	constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 4, h), new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, 15));
...
mxLeanTimeline.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const shapeInfo = mxUtils.getValue(this.style, 'mainText', '20,Time 1,50,Time 2,30,Time 3,40,Time 4,30,Time 5,50,Time 6,20,Time 7').toString().split(',');
	...
	{
		var currTime = Number.parseFloat(shapeInfo[i]); 
		times.push(currTime);
...
mxLeanElectronicInfoShape.prototype.paintEdgeShape = function(c, pts)
{
	...
	const p1x = midX - nx * dist * 0.1 - ny / 3 * dist * 0.1;
	const p1y = midY - ny * dist * 0.1 + nx / 3 * dist * 0.1;
	const p4x = midX + nx * dist * 0.1 + ny / 3 * dist * 0.1;
	const p4y = midY + ny * dist * 0.1 - nx / 3 * dist * 0.1;
...
mxShapeLeanTimeline.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const dy1 = mxUtils.getValue(this.style, 'dy1', '0');
	const dx2 = mxUtils.getValue(this.style, 'dx2', '0');
	const dy2 = mxUtils.getValue(this.style, 'dy2', '0');
	const dx3 = mxUtils.getValue(this.style, 'dx3', '0');
	const dy3 = mxUtils.getValue(this.style, 'dy3', '0');
	const dx4 = mxUtils.getValue(this.style, 'dx4', '0');
	const dy4 = mxUtils.getValue(this.style, 'dy4', '0');
	const dx5 = mxUtils.getValue(this.style, 'dx5', '0');
	const dy5 = mxUtils.getValue(this.style, 'dy5', '0');
	const dy6 = mxUtils.getValue(this.style, 'dy6', '0');
...
{
	const handles = [Graph.createHandle(state, ['dy1'], function(bounds)
			{
	...
	const handle3 = Graph.createHandle(state, ['dx3', 'dy3'], function(bounds)
			{
	...
	const handle4 = Graph.createHandle(state, ['dx4', 'dy4'], function(bounds)
			{
	...
	const handle5 = Graph.createHandle(state, ['dx5', 'dy5'], function(bounds)
			{
	...
	const handle6 = Graph.createHandle(state, ['dy6'], function(bounds)
			{
