...
mxShapeElectricalTwoLineBusElbow.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const wn = Math.min(w, notch);
	const hn = Math.min(h, notch);
...
{
	const handles = [Graph.createHandle(state, ['notch'], function(bounds)
	{
...
{
	c.translate(x, y);
	const numInputs = parseInt(mxUtils.getValue(this.style, 'numInputs', '2'));
	const spacing = h / numInputs;
	let currH = spacing * 0.5;

	c.begin();
	c.moveTo(w * 0.8, h * 0.5);
	c.lineTo(w, h * 0.5);

	const operation = mxUtils.getValue(this.style, 'operation', 'and');
	...
	const negating = mxUtils.getValue(this.style, 'negating', '0');
	...
	if (negating == '1')
	{
		...
		{
			const tmpSize = parseFloat(mxUtils.getValue(this.style, 'negSize', '0.13'));
...
{
	const constr = [new mxConnectionConstraint(new mxPoint(1, 0.5), false)];
	
	const numInputs = parseInt(mxUtils.getValue(style, 'numInputs', '2'));
	const spacing = 1 / numInputs;
	let currH = spacing * 0.5;
...
mxShapeElectricalBuffer.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	if (negating == '1')
	{
		const negSize = Math.min(w * 0.04, h * 0.07);
		c.begin();
		c.ellipse(w * 0.8, h * 0.5 - negSize * 0.5, negSize, negSize);
		c.fillAndStroke();
	}
...
{
	c.translate(x, y);
	const fontColor = mxUtils.getValue(this.style, 'fontColor', '#000000');
	c.setFontColor(fontColor);
	const startPin = mxUtils.getValue(this.style, 'startPin', 'n');
	const pinLabelType = mxUtils.getValue(this.style, 'pinLabelType', 'gen');
	const labelNames = decodeURIComponent(mxUtils.getValue(this.style, 'labelNames', '').toString()).split(',');
	...
	const pinSpacing = parseFloat(mxUtils.getValue(this.style, 'pinSpacing', '20'));
	const pinStyle = mxUtils.getValue(this.style, 'pinStyle', 'line');
	const fontSize = parseFloat(mxUtils.getValue(this.style, 'fontSize', '12'));
	
	if (startPin == 'n' || startPin == 's')
	{
		var pinsOne = Number.parseInt(h / pinSpacing);
	}
	else
	{
		var pinsOne = Number.parseInt(w / pinSpacing);
	}
...
mxShapeElectricalDualInLineIC.prototype.getConstraints = function(style, w, h)
{
	...
	const startPin = mxUtils.getValue(this.style, 'startPin', 'n');
	let pinCount = 1;
	let currH = pinSpacing * 0.5;
	const pinsOne = parseInt(h / pinSpacing);
	
	if (startPin == 'n' || startPin == 's')
	{
		while (pinCount * pinSpacing <= h)
		{
			constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, currH), new mxConnectionConstraint(new mxPoint(1, 0), false, null, 0, currH));
			currH = currH + pinSpacing;
			pinCount++;
		}
	}
	else
	{
		while (pinCount * pinSpacing <= w)
		{
			constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, currH, 0), new mxConnectionConstraint(new mxPoint(0, 1), false, null, currH, 0));
			currH = currH + pinSpacing;
			pinCount++;
		}
	}
...
mxShapeElectricalQFPIC.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const fontColor = mxUtils.getValue(this.style, 'fontColor', '#000000');
	c.setFontColor(fontColor);
	const startPin = mxUtils.getValue(this.style, 'startPin', 'sw');
	...
	if (pinStyle == 'line')
	{
		...
		var pinsVOne = Number.parseInt((h - pinSpacing - 40) / pinSpacing) + 1;
		var pinsHOne = Number.parseInt((w - pinSpacing - 40) / pinSpacing) + 1;
...
{
	c.translate(x, y);
	const selectorPins = parseInt(mxUtils.getValue(this.style, 'selectorPins', '1'));
	const operation = mxUtils.getValue(this.style, 'operation', 'mux');
	const fontSize = parseFloat(mxUtils.getValue(this.style, 'fontSize', '12'));
	c.setFontSize(fontSize * 0.5);
	const fontColor = mxUtils.getValue(this.style, 'fontColor', '#000000');
	c.setFontColor(fontColor);
	const dir = mxUtils.getValue(this.style, 'direction', 'east');
	let txtRot = 0;
	...
	if (operation == 'mux')
	{
		numInputs = 2 ** selectorPins;
		var spacing = (h - 16) / numInputs;
	}
	else
	{
		numOutputs = 2 ** selectorPins;
		var spacing = (h - 16) / numOutputs;
	}
	
	let currH = 3 + spacing * 0.5;
	...
	const spacing = (w - 20) / selectorPins;
	let currW = 10 + spacing * 0.5;
...
mxShapeElectricalMux.prototype.getConstraints = function(style, w, h)
{
	...
	const spacing = (w - 20) / (w * selectorPins);
	let currW = spacing * 0.5;
...
mxShapeElectricalBatteryStack.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const bw = h * 0.3;
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '#000000');
	const dashed = mxUtils.getValue(this.style, mxConstants.STYLE_DASHED, '0');
	
	const bNum = Math.floor((w - 20) / bw);
	const startX = (w - bNum * bw) * 0.5;
...
mxShapeElectricalDCSource3_v2.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const ss = Math.max(3, Math.min(h, w) * 0.05); // half of symbol size
	const i = 3; //indent
	c.ellipse(0, 0, w, h);
	c.fillAndStroke();
...
mxShapeElectricalSource.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const sourceType = mxUtils.getValue(this.style, 'elSourceType', 'independent');
	const signalType = mxUtils.getValue(this.style, 'elSignalType', 'none');
...
{
	c.translate(x, y);
	const switchState = mxUtils.getValue(this.style, 'elSwitchState', '1');
...
{
	c.translate(x, y);
	const switchState = mxUtils.getValue(this.style, 'elSwitchState', 'on');
...
{
	c.translate(x, y);
	const switchState = mxUtils.getValue(this.style, 'elSwitchState', 'neutral');
...
{
	c.translate(x, y);
	const switchState = mxUtils.getValue(this.style, 'elSwitchState', 'off');
