...
{
	const mainText = mxUtils.getValue(this.style, 'buttonText', 'Entity');
	const fontColor = mxUtils.getValue(this.style, 'textColor', '#666666');
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '17');
	c.translate(x, y);
	const rSize = 10;
...
{
	const buttonStyle = mxUtils.getValue(this.style, 'buttonStyle', 'round').toString();
...
{
	const mainText = mxUtils.getValue(this.style, 'buttonText', 'Entity');
	const attributes = mxUtils.getValue(this.style, 'subText', '+ attribute 1,+ attribute 2,+ attribute 3').toString().split(',');
	const strokeColor = mxUtils.getValue(this.style, mxConstants.STYLE_STROKECOLOR, '#666666');
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '17');
	const mainColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '#008cff');
	const attrColor = mxUtils.getValue(this.style, 'fillColor2', '#ffffff');
	let maxTextWidth = 0;
	c.translate(x, y);
	const rSize = 10;
	const barY = fontSize * 1.25;

	for (let i = 0; i < attributes.length; i++)
	{
		const currWidth = mxUtils.getSizeForString(attributes[i], fontSize, mxConstants.DEFAULT_FONTFAMILY).width;
...
{
	const buttonStyle = mxUtils.getValue(this.style, 'buttonStyle', 'simple').toString();
	...
	{
		const fillColor = mxUtils.getValue(this.style, mxConstants.STYLE_FILLCOLOR, '#666666');
		c.setFillColor(fillColor);
		c.begin();
		c.ellipse(0, 0, w, h);
		c.fillAndStroke();
...
{
	const buttonStyle = mxUtils.getValue(this.style, 'buttonStyle', 'rhombus').toString();
...
{
	const mainText = mxUtils.getValue(this.style, 'buttonText', 'main').toString().split(',');
	const subText = mxUtils.getValue(this.style, 'subText', 'sub').toString().split(',');
	const fontColor = mxUtils.getValue(this.style, 'textColor', '#666666');
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '17');
	c.translate(x, y);
	const rSize = 10;
...
mxShapeERNote.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const fontColor = mxUtils.getValue(this.style, 'textColor', '#666666');
	const fontSize = mxUtils.getValue(this.style, mxConstants.STYLE_FONTSIZE, '17');
	const backColor = mxUtils.getValue(this.style, 'fillColor2', '#ffffff');
	c.translate(x, y);
	const flipSize = 20;
...
mxShapeERBachman.prototype.foreground = function(c, x, y, w, h, fontSize, fontColor)
{
	...
	const textWidth = mxUtils.getSizeForString('has/forms', fontSize, mxConstants.DEFAULT_FONTFAMILY).width;
	c.begin();
...
mxMarker.addMarker('ERzeroToMany', function(c, shape, type, pe, unitX, unitY, size, source, sw, filled)
{
	...
	return function()
	{
		...
		if (filled)
		{
			...
			const oldColor = mxUtils.getValue(shape.style, mxConstants.STYLE_STROKECOLOR, '#666666');
...
mxShapeERRRect.prototype.paintVertexShape = function(c, x, y, w, h)
{
	...
	const rSize = parseInt(mxUtils.getValue(this.style, mxShapeERRRect.prototype.cst.R_SIZE, '10'));
