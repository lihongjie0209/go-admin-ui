...
mxShapeEmoji.prototype.getUnicodeText = function(emoji)
{
    ...
    if (emoji != '')
    {
        ...
        {
            for (let comp in this.emojiComponents)
            {
                if (emoji.length > comp.length && emoji.indexOf(comp) == emoji.length - comp.length)
                {
                    ...
                    if (emojiUnicode != null)
                    {
                        ...
                        {
                            emojiUnicode = emojiUnicode.slice(0, 2) + this.emojiComponents[comp] + emojiUnicode.slice(2);
                        }
                        else
                        {
...
{
    const emoji = mxUtils.getValue(this.style, this.EMOJI_NAME, '');
    const emojiUnicode = this.getUnicodeText(emoji) || '';

	c.rect(x, y, w, h);
    c.fillAndStroke();
    const origSize = c.state.fontSize;
