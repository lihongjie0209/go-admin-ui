...
export const authenticateResponseInterceptor = ({
  ...
  const reauthenticateOnce = async () => {
    if (!reauthenticatePromise) {
  ...
  return {
    rejected: async (error) => {
      ...
      // Every failed request is marked before waiting so a replay can never
      // recursively enter the refresh flow. Concurrent requests share the
      // same promise and are all rejected when refresh fails; none are replayed
      // with an empty or stale credential.
